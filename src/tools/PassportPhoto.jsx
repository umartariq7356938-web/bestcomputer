...
export default function PassportPhoto({ tool }) {
  ...
  const processPhoto = async () => {
    ...
    try {
      ...
      // Target pic dimensions (1.35" x 1.85")
      const picW = Math.round(1.35 * dpi); // 405px
      const picH = Math.round(1.85 * dpi); // 555px
      ...
      // Landscape (6x4): 4 cols, 2 rows (8 pics)
        ...
        <>
          <label className="upload-area">
            ...
            <p style={{ color: '#607082', marginTop: '10px' }}>
              Upload a clear, straight-facing portrait photo.
            </p>
      ...
      {state === 'success' && (
        <div style={{ textAlign: 'center', padding: '30px 0' }}>
          ...
          <div style={{ background: '#f8fafc', padding: '20px', borderRadius: '12px', display: 'inline-block', border: '1px solid var(--line)' }}>
             <img src={resultUrl} alt="Passport Sheet Preview" style={{ maxWidth: '100%', maxHeight: '400px', objectFit: 'contain', boxShadow: '0 4px 12px rgba(0,0,0,0.1)' }} />
          </div>

          <div style={{ marginTop: '30px', display: 'flex', gap: '10px', justifyContent: 'center' }}>
            <a href={resultUrl} download={`PassportSheet_${orientation}_${Date.now()}.jpg`} className="button">
